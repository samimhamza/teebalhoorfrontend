<template>
  <v-container fluid class="pa-0">
    ...
    <v-dialog
      ...
      width="1100"
      persistent
      v-if="$isInScope('employees-create')"
    >
    ...
    <v-dialog
      ...
      max-width="1400"
      width="1400"
      v-if="$isInScope('employees-view')"
    >
      <UserFilter @close="onFilter" @getRecord="getRecord" />
    </v-dialog>
    <v-dialog
      v-model="showProfile"
      persistent
    ...
    <!-- User Bulk Upload -->
    <v-dialog v-model="userBulkUpload" width="1100">
      <Dialog persistent max-width="1200" @closeDialog="userBulkUpload = false">
    ...
    <v-dialog
      ...
      persistent
      width="1200"
      v-if="$isInScope('employees-update')"
    >
      <UserEdit
    ...
    <v-row no-gutters>
      ...
      <v-col cols="12">
        ...
        >
          <template slot="button">
            <CustomButton
              v-if="$isInScope(['employees-view'], true)"
              :icon="
                this.onlineVisible
          ...
          <template slot="content" v-if="$isInScope(['employees-view'], true)">
      ...
      <v-col cols="12">
        <PageFilters
          ...
          :filename="$tr('users_list')"
          :show-add-note="$isInScope('employees-create')"
          :show-bulk-upload="$isInScope('employees-create')"
          :showDownload="$isInScope('employees-view')"
      ...
      <v-col cols="12">
        <PageActions
          :showTracing="true"
          :showView="$isInScope('employees-view')"
          :showEdit="$isInScope('employees-update')"
          :showAutoEdit="$isInScope('employees-update')"
          :showDelete="$isInScope('employees-delete')"
          :showBlock="$isInScope('employees-update')"
          :showSelect="$isInScope('employees-update')"
          :showApply="$isInScope('employees-update')"
...
export default {
  ...
  watch: {
    tabIndex: function (value) {
      ...
      this.fetchItems({ key: this.getTabKey(), options: this.filterData });
    },
  },
  computed: {
    ...mapGetters({
  ...
  methods: {
    ...
    onTableChanges(options) {
      this.fetchItems({ key: this.getTabKey(), options: options });
    },
    ...
    async onBlock() {
      ...
      this.getTabKey() === "pending"
        ? this.noReasonSubmit()
        : ((this.showReasonDialog = true), await this.fetchAllReasons());
    },
    ...
    fetchRecentDataAndClearSelected() {
      ...
      this.userIds = [];
      this.selectedStatus = "";
      this.fetchItems({ key: this.getTabKey() });
    },
