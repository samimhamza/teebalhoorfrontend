...
export default {
  ...
  async created() {
    this.form = {
      subsystem: this?.prevFilterData?.subsystem ? this.prevFilterData?.subsystem : "",
      ...
      created_date: this?.prevFilterData?.created_date
        ...
        : "",
      title: this?.prevFilterData?.title
        ? this.prevFilterData?.title
        : "",
      label_ids:
        this?.prevFilterData?.label_ids?.length > 0 ? this.prevFilterData?.label_ids : [],
    };
  },
  computed: {},
  methods: {
    addId() {
      if (!this.form.label_ids.includes(this.label_id)) {
        this.form.label_ids.push(this.label_id);
      }
      this.label_id = "";
    },
    clearId(id) {
      this.form.label_ids = this.form.label_ids.filter(
        (element) => element !== id
      );
    },
    closeDialog() {
