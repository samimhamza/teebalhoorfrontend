    ...
    <v-card-text>
      ...
      <v-row class="mt-4">
        <v-col cols="12" class="d-flex py-0">
          <v-card elevation="0" class="pa-2 w-full">
            <div>
              <span class="subtitle-1">{{ $tr("details") }} </span>
              <v-divider class="grey darken-1 mt-1 mb-2"></v-divider>
            </div>
            <v-row>
              <v-col cols="6" class="py-0">
                <span class="subtitle-1">{{ $tr("label") }} </span>
                <div class="text-primary" style="font-size: 1rem">
                  {{ profileData.label }}
                </div>
              </v-col>
              <v-col cols="3" class="py-0">
                <span class="subtitle-1">{{ $tr("status") }} </span>
                <div class="text-primary" style="font-size: 1rem">
                  {{ profileData.status }}
                </div>
              </v-col>
              <v-col cols="3" class="py-0">
                <span class="subtitle-1">{{ $tr("color") }} </span>
                <div class="text-primary" style="font-size: 1rem">
                  {{ profileData.color }}
                </div>
              </v-col>
      ...
      <v-row class="mt-4">
        <v-col cols="12" class="d-flex py-0">
          <v-card elevation="0" class="pa-2 w-full">
            ...
            <v-row>
              <template v-for="(sub, index) in profileData.sub_systems">
                <v-col cols="12" md="3" sm="4" class="py-0" :key="index">
                  <v-checkbox
                    :label="$tr(sub.name)"
                    hide-details
                    input-value="true"
