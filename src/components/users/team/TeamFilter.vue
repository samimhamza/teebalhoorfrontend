  ...
  created() {
    this.form = {
      ...
      department_id: this?.prevFilterData?.department_id
        ...
        : "",
      team_ids:
        this?.prevFilterData?.team_ids?.length > 0
          ? this.prevFilterData?.team_ids
          : [],
    };
  },
  methods: {
    ...mapActions({}),
    addId() {
      if (!this.form.team_ids.includes(this.team_id)) {
        this.form.team_ids.unshift(this.team_id);
      }
      this.team_id = "";
    },
    clearId(id) {
      this.form.team_ids = this.form.team_ids.filter(
        (element) => element !== id
      );
    },
    closeDialog() {
