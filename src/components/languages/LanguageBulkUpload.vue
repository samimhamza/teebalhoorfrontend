      ...
      <template v-slot:fileUpload>
        ...
        >
          <template v-slot:[`item.name`]="{ item, index }">
            <span :class="`${checkIfHasErr(item.name, 'name', index+6) ? 'text--white error-class px-2 d-flex align-center justify-center' : ''}`">
                {{ item.name ? item.name : 'Empty' }}
            </span>
          </template>
          <template v-slot:[`item.logo`]="{ item, index }">
            <span :class="`${checkIfHasErr(item.logo, 'logo', index+6) ? 'text--white error-class px-2 d-flex align-center justify-center' : ''}`">
              {{ item.logo ? item.logo : 'Empty' }}
            </span>
          </template>>
          <template v-slot:[`item.country`]="{ item, index}">
            <span :class="`${checkIfHasErr(item.country, 'country', index+6) ? 'text--white error-class px-2 d-flex align-center justify-center' : ''}`">
              {{ item.country ? item.country : 'Empty' }}
            </span>
          </template>
          <template v-slot:[`item.note`]="{ item, index }">
            <span
                :class="`${checkIfHasErr(item.note, 'note', index+6) ? 'text--white error-class px-2 d-flex align-center justify-center' : ''}`"
            >
          ...
          <template v-slot:[`item.label`]="{ item, index }">
            <span
                :class="`${checkIfHasErr(item.label, 'label', index+6) ? 'text--white error-class px-2 d-flex align-center justify-center' : ''}`"
            >
          ...
          <template v-slot:[`item.company`]="{ item,index }">
            <span
                :class="`${checkIfHasErr(item.company, 'company', index+6) ? 'text--white error-class px-2 d-flex align-center justify-center' : ''}`"
            >
  ...
  methods: {
    checkIfHasErr(value, col, index){
        ...
        if(value){
            this.errors.forEach(e => {
                if(typeof e.row =='object' ? (e.row.includes(index) && e.column === col) : (e.row == index && e.column === col)){
                    hasErr = true;
                    return;
                }
            });
    ...
    async onSubmit(){
      if (this.selectedRows.length) {
        ...
        try {
          const response = await this.$axios.post("/language-bulk-upload", {langs: finalData, lang_id:this.language_fetched.id});
    ...
    async onDownload() {
      try {
        ...
        const response = await this.$axios.get("/export-language-template", {params: lang});
